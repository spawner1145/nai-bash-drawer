...
async def n3(prompt, output_type, output_dir, zip_file, filename):
    global round_nai
...
async def nai3(tag, output_type, output_dir, zip_file):
    ...
    while retries_left > 0:
        try:
            generated_filename = await n3(tag, output_type, output_dir, zip_file, filename)
...
async def main():
    ...
    try:
        for _ in range(NUM_IMAGES):
            await nai3(AAA_TAGS, OUTPUT_TYPE, OUTPUT_DIR, zip_file)
